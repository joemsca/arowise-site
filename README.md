...
| `agency.html`, `small-business.html` | **Generated build artifacts. Never hand-edit.** The agency and small-business services menus, rendered from the private vault by `skills/automation_services_catalog/scripts/render_menu.py`; the words live in that repo's `catalog.json`. Each is one self-contained file sharing nothing with `styles.css` or `site.js`, and neither is linked from any page. `/agency` carries `noindex`; `/small-business` does not. Re-render and re-commit after any catalog edit or they go stale silently. |
